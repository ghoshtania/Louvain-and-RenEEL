...
class Community {
 public:
  ...
  vector<double> ini,tot; // used to compute the modularity participation of each community
  ...
  double modularity();
  double partitioning(int which_node);
...
Community::remove(int node, int comm, double dnodecomm) {
  ...
  ini[comm]  -= 2*dnodecomm + g.nb_selfloops(node);
  n2c[node]  = -1;
}
